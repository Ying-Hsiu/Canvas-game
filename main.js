...
function gameOver(){
    ...
    reset = setInterval(function(){
        ...
        text('red','→ Press \'R\' key to back Title ←',90,160,'15pt');
        if(health>0){
            text('red','or',195,180,'15pt');
            text('red','→ Press \'ENTER\' key to back game ←',70,200,'15pt');  
        }
        text('#FFF','YOUR SCORE: '+scores,145,240,'15pt');
    },50);
}
...
//積分表
function score(){
    text('red','YOUR SCORE:'+scores,10,25,'15pt');
    text('#FFF','Press \'Q\' to Quit',280,380,'15pt');
}
...
function text(color,str,x,y,size){
    ctx_2.fillStyle=color;
    ctx_2.font=size+" bold 'orange kid'";
...
function start(){
        ...
        moveBall();
        text('#FFF','→ Press \'ENTER\' key to start game ←',70,348,'15pt');
        text('#C30','THE EARTH',90,100,'45pt');
        text('#C30','DEEP IMPACT',160,120,'15pt');
        text('#FFF','Difficulty Level: '+level,130,200,'15pt');
    },100);
}
var j = start();
